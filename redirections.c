...
/*   Updated: 2021/05/24 16:46:21 by hchorfi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_out_red_file(char *tmp_out, char **new_pipe, int i)
{
	char		*file;
	char		*tmp_free;
	int			j;
...
char	*ft_check_redirections(char *pipe_cmds, int i, char *new_pipe)
{
	...
	while (str[i])
	{
		...
		else
		{
			...
			ft_out_red_file(tmp_in, &new_pipe, 0);
		}
	}
	ft_free_d_p(str);
	return (new_pipe);
